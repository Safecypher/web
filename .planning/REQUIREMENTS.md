...
- [ ] **FOUND-01**: Next.js 14+ App Router project initialised with TypeScript strict mode, folder structure matching PRD (`app/`, `components/ui`, `components/marketing`, `components/portal`, `lib/`)
- [ ] **FOUND-02**: Netlify deployment configured with @netlify/plugin-nextjs; `main` → production, `develop` → staging
- [ ] **FOUND-03**: GitHub Actions CI/CD pipeline runs lint, type-check, and build on every PR; failing checks block merge
...
- [ ] **SV-08**: Outbound flow diagram (SVG placeholder; production SVG to replace PDF in Phase 1)
